...
base_apps+=("extra|git|$git_baseurl/apps/extra.git|$git_branch")
...
extra_modules+=("alarm|git|$git_baseurl/enlightenment/modules/alarm.git|$git_branch")
extra_modules+=("cpu|git|$git_baseurl/enlightenment/modules/cpu.git|$git_branch")
extra_modules+=("diskio|git|$git_baseurl/enlightenment/modules/diskio.git|$git_branch")
extra_modules+=("edgar|git|$git_baseurl/enlightenment/modules/edgar.git|$git_branch")
extra_modules+=("extramenu|git|$git_baseurl/enlightenment/modules/extramenu.git|$git_branch")
extra_modules+=("forecasts|git|$git_baseurl/enlightenment/modules/forecasts.git|$git_branch")
extra_modules+=("mail|git|$git_baseurl/enlightenment/modules/mail.git|$git_branch")
extra_modules+=("moon|git|$git_baseurl/enlightenment/modules/moon.git|$git_branch")
extra_modules+=("mpdule|git|$git_baseurl/enlightenment/modules/mpdule.git|$git_branch")
extra_modules+=("net|git|$git_baseurl/enlightenment/modules/net.git|$git_branch")
extra_modules+=("news|git|$git_baseurl/enlightenment/modules/news.git|$git_branch")
extra_modules+=("penguins|git|$git_baseurl/enlightenment/modules/penguins.git|$git_branch")
extra_modules+=("places|git|$git_baseurl/enlightenment/modules/places.git|$git_branch")
extra_modules+=("tclock|git|$git_baseurl/enlightenment/modules/tclock.git|$git_branch")
declare -a extra_libs
extra_libs+=("python-efl|git|$git_baseurl/bindings/python/python-efl.git|$git_branch")
declare -a extra_apps
extra_apps+=("econnman|git|$git_baseurl/apps/econnman.git|$git_branch")
extra_apps+=("edi|git|$git_baseurl/tools/edi.git|$git_branch")
extra_apps+=("elm-theme-viewer|git|$git_baseurl/tools/elm-theme-viewer.git|$git_branch")
extra_apps+=("enjoy|git|$git_baseurl/apps/enjoy.git|$git_branch")
extra_apps+=("enventor|git|$git_baseurl/tools/enventor.git|$git_branch")
extra_apps+=("ephoto|git|$git_baseurl/apps/ephoto.git|$git_branch")
extra_apps+=("expedite|git|$git_baseurl/tools/expedite.git|$git_branch")
extra_apps+=("rage|git|$git_baseurl/apps/rage.git|$git_branch")
...
declare -a extra_e16
extra_e16+=("imlib2|git|$git_baseurl/legacy/imlib2.git|$git_branch")
extra_e16+=("imlib2_loaders|git|$git_baseurl/legacy/imlib2_loaders.git|$git_branch")
extra_e16+=("e16|git|$git_baseurl/e16/e16.git|$git_branch")
extra_e16+=("e16-epplets|git|$git_baseurl/e16/e16-epplets.git|$git_branch")
extra_e16+=("e16-keyedit|git|$git_baseurl/e16/e16-keyedit.git|$git_branch")
extra_e16+=("e16-menuedit|git|$git_baseurl/e16/e16-menuedit.git|$git_branch")
extra_e16+=("e16-themes|git|$git_baseurl/e16/e16-themes.git|$git_branch")
...
for package in ${extra_modules[*]}
{	package_name=`echo "$package" | cut -d'|' -f1`
	extra_modules_list="$extra_modules_list $package_name"
}
for package in ${extra_libs[*]}
{	package_name=`echo "$package" | cut -d'|' -f1`
	extra_libs_list="$extra_libs_list $package_name"
}
for package in ${extra_apps[*]}
{	package_name=`echo "$package" | cut -d'|' -f1`
	extra_apps_list="$extra_apps_list $package_name"
}
for package in ${extra_games[*]}
{	package_name=`echo "$package" | cut -d'|' -f1`
	extra_games_list="$extra_games_list $package_name"
}
for package in ${extra_e16[*]}
{	package_name=`echo "$package" | cut -d'|' -f1`
	extra_e16_list="$extra_e16_list $package_name"
}

# Basic Dependencies
deps_bin="automake autopoint byacc|yacc dbus-launch g++ gcc git libtool pkg-config pulseaudio"
deps_dev="blkid BulletSoftBody check dbus-1 fontconfig freetype fribidi gif GL gstreamer-1.0 gstvideo-1.0 jpeg lua|lua5.1|lua5.2|lua5.3 luajit-5.1 mount png pam poppler poppler-cpp pulse raw rsvg-2 sndfile spectre ssl|gnutls systemd tiff udev xml2 X11 xcb xcb-keysyms xcb-shape Xcursor Xcomposite Xext Xinerama Xrandr Xss Xtst"
...
animation="star"
online_source="http://omicron.homeip.net/svn/easy_efl/easy_efl.sh"
...
function package_find ()
{
	...
	found=0
	for package in ${base_libs[*]} ${base_apps[*]} ${extra_modules[*]} ${extra_libs[*]} ${extra_apps[*]} ${extra_games[*]} ${extra_e16[*]} ${custom_packages[*]}
	{
		package_name=`echo "$package" | cut -d'|' -f1`
		...
		case $1 in
			0)
				if [ "$os" == "not supported" ]; then
					...
					echo "                           - complete     (efl+enlightenment+e-modules+apps+e16+custom)"
					...
					echo "                                          an error"
					echo "      --skip-depcheck                   = continue on dependency errors"
			...
			1)
				...
				;;
			2)
				echo "- libs compilation and installation"
...
# create packagelists
packages_basic="$base_libs_list$base_apps_list$custom_packages_list"
packages_full="$base_libs_list$base_apps_list$extra_modules_list$extra_libs_list$extra_apps_list$extra_games_list$custom_packages_list"
packages_complete="$base_libs_list$base_apps_list$extra_modules_list$extra_libs_list$extra_apps_list$extra_games_list$extra_e16_list$custom_packages_list"
...
do
	...
	if [ "$option" == "--packagelist" ]; then
		case $value in
			...
			"complete")			packages="$packages_complete"	;;
		esac
	fi 
done
...
if [ ! "$action"  == "srcupdate" ]; then
	...
	if [ "$only" ]; then
		echo
		echo "- matching packages with package lists (basic/full/complete/custom)..."
